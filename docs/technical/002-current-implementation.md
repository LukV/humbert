...
How Humbert's code works today, written for a person who needs to understand it — a new contributor, a future maintainer, or a Claude session that shouldn't have to reverse-engineer 3,000 lines to find a seam. The *why* behind these choices lives in `../architecture/001-stack-decisions.md`; the operator's guide for running a data pack is `001-information-manager-instructions.md`. This note is the *what*.
...
- `Selection` — the reproducible IR of a question (metrics + group_by + where + order/limit/grain). Serialisable on purpose: this is what a validated cell will freeze.
