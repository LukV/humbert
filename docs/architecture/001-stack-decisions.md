...
- **Persistence at `~/.humbert/`** (*nix) and the OS equivalent on Windows (e.g. `%LOCALAPPDATA%\humbert`, via a platform-dirs helper). Layout mirrors Lumen's `~/.lumen/`:
  - `config.json` — connections (now dbt projects, each with an `active_pack`), `active_connection`, `llm`, `settings` (`max_result_rows`, `statement_timeout_seconds`, `theme`, `telemetry_enabled`).
...
- **Agent framework.** How plan / run / narrate (and any agent steps) are structured. **PydanticAI** is the leading candidate. Decide before block 2 — it shapes the orchestration code.
